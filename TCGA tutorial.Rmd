...
```{r}
# Q1: complete function is used to remove missing values. fill=list(n=0) defines that 0 takes the place of missing values.

# Q2: facet_wrap is used to arrange plots side-by-side. 
## ~type defines the row group name/ scales='free_x' defines the scale is free in the x demension/ ncol defines column numbers
```
...
```
```{r}
#the x-axis is a day scale. There seems to be two humps on this data so it doesn't follow normal distribution.
```
...
```{r}
# answer : I think it's OK. If the mean and IQR range was applied also, it would convey more information. Also a little explanation about the two bumps formed will be needed.
```
...
```{r}
# boxplot has median and IQR values.
```
...
```{r}
# I think violin plots are a tilted version of density plots
```
...
bind_rows(d_luad%>%
          ...
          d_lusc%>%
            ...
            select(type,race))%>%
  count(type,race)%>%
  complete(type, race, fill = list(n = 0))%>%
  ggplot(aes(race,n))+
    geom_bar(stat="identity")+
  facet_wrap(~type)+coord_flip()+
  labs(y="",x="Race",title = "Race distribution in LUAD and LUSC")
```
...
```{r}
bind_rows(d_luad%>%
            filter(shortLetterCode=="TP",!(race%in%c("not reported","american indian or alaska native")))%>%
            mutate(type="luad")%>%
            select(type,race,gender,age_at_diagnosis),
          d_lusc%>%
            filter(shortLetterCode=="TP",race!="not reported")%>%
            mutate(type="lusc")%>%
            select(type,race,gender,age_at_diagnosis))%>%
  ggplot(aes(gender,age_at_diagnosis/365,fill=gender))+geom_boxplot()+
  facet_wrap(~race)+
  labs(y="Year at diagnosis",x="")
```
...
# save to file
ggsave('plot.TCGA_race_boxplot.pdf', p, width = 16, height = 9)
```
...
```{r}
#cigarettes per day of lusc patients
d_lusc%>%
  filter(shortLetterCode=="TP")%>%
  ggplot(aes(gender,cigarettes_per_day))+
  geom_violin(aes(fill=gender)) +   geom_boxplot(fill='white', width=0.25)+
  scale_y_continuous(trans = "sqrt")+
  labs(y="cigarettes per day(sqrt)",title="Cigarettes per day of LUSC patients")
```
...
counts_tumor2<-d_lusc%>%count(tumor_stage)
counts_tumor2
```
...
```{r}
#lusc is found in males more frequently
```
