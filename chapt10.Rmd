...
![(Source: Fox News, via Media Matters)](images/class2_8.jpeg)
...
![](images/points-plot-not-from-zero-1.png)
...
![(Source: The 2011 State of the Union Address)](images/state-of-the-union.png)
...
![](images/barplot-better-than-area-1.png)
...
![](images/do-not-order-alphabetically-1.png)
...
![](images/reorder-boxplot-example-1.png)
...
![](images/common-axes-histograms-wrong-1.png)
...
![](images/common-axes-histograms-right-1.png)
...
![](images/show-the-data-comparison-1.png)
...
![](images/no-transformations-wrong-use-of-barplot-1.png)
...
![](images/no-transformation-1.png)
...
![](images/correct-transformation-1.png)
...
![](images/boxplots-not-adjacent-1.png)
...
![](images/boxplot-adjacent-comps-1.png)
...
![](images/boxplot-adjacent-comps-with-color-1.png)
...
Here are the colors
![](images/color-blind-friendly-colors-1.png)
...
![](images/scatter-plot-instead-of-slope-1.png)
...
![](images/encoding-third-variable-1.png)
...
# 10.10 Avoid pseudo-three-demensional plots
...
![(Image courtesy of Karl Broman)](images/fig8b.png)
...
![](images/colors-for-different-lines-1.png)
...
![](images/fig1e.png)
![(Images courtesy of Karl Broman)](images/fig2d.png)
...
```{r}
# answer: d.Never. Barplots and tables are always better.
```

2. What is the problem with the plot below:

![](images/baplot-not-from-zero-exercises-1.png)
...
![](images/pseudo-3d-exercise-1.png)
...
![](images/pseud-3d-exercise-2-1.png)
...
```{r}
#filter data for smallpox(case reported for more than 10weeks)
...
avg2<-us_contagious_diseases%>%
  filter(disease=="Smallpox")%>%
  group_by(year)%>%
  summarize(us_rate2=sum(count,na.rm = TRUE)/
              sum(population,na.rm = TRUE)*10000)
...
dat2%>%
  filter(!is.na(rate))%>%
  ggplot()+
  geom_line(aes(year,rate,group=state),alpha=0.3,color="grey50")+
  geom_line(mapping=aes(year,us_rate2),data=avg2,col="blue")+
  geom_text(data=data.frame(x=1930,y=4.5),mapping=aes(x,y,label="US average"))+
...
datC%>%filter(!is.na(rate))%>%
  ggplot()+
  geom_line(aes(year,rate,col=disease))+
  scale_y_continuous(trans="sqrt",breaks=c(50,100))+
  theme(legend.position ="bottom",legend.title = element_blank())+
  geom_vline(xintercept = 1963,color="black")+
  geom_text(data=data.frame(x=1975,y=50),mapping = aes(x,y,label="Vaccine introduced"),col="black",size=3)+
  xlab("")+ylab("Cases per 10,000")+
  ggtitle("California Disease cases per 10,000")
```
...
us_contagious_diseases%>%
  ...
  geom_text(data=data.frame(x=1975,y=50),mapping = aes(x,y,label="Vaccine introduced"),col="black",size=3)+
  xlab("")+ylab("Cases per 10,000")+
  ggtitle("US Disease cases per 10,000")
