...
```{r}
summary(d)
#answer: PatientID, PatientSex, PatientAgeAtAssessment, Pos_hg19, Ref, Alt, Type, Effect, c.DNA, p.Protein, Inheritence, Source, SourcePMID, Ben-Shalom2017, Wolff2017,UniqueSample/Family, Seizures, SeizureOnsetDays, SeizureType, ASD, DD/ID, DD/ID severity, OtherFeature, Classification
```
...
#answer: numeric values change to characters
```
...
d1$SeizureOnsetDays[is.na(d1$SeizureOnsetDays2=="TRUE")]
#They all have <
```
...
library(tidyverse)
head(d%>%mutate(Ptnnumb=gsub('[^0-9]','',d$p.Protein)))
...
cDNA<-as.data.frame(table(d$c.DNA))
cDNA

#check the recurrent mutations for each group
...
```{r}
length(unique(d$Classification))
unique(d$Classification)

# 7 different types of phenotypes occur

#calculating proportion

##total cases=293

c<-as.data.frame(table(d$Classification))

c%>% mutate(proportion=Freq/293)
...
sy<-d%>%
  ...
  group_by(PatientSex,Classification)%>%count()

sy%>%mutate(proportion=n/293)
...
##mutation "consequences" are loss-of-function (Nonsense, Frameshift) or missense. 

syd<-d%>%
  filter(Effect%in%c("Missense","Nonsense","Frameshift"))%>%
  group_by(Classification,Effect)%>%count()
syd


##how many mutation consequences are observed in each phenotype
table(syd$Classification)
```
...
cd<-d%>%
  mutate(loci=gsub('[^0-9]','',d[1:293,'c.DNA']))%>%
  group_by(loci,c.DNA)%>%
  count()
new <- as.data.frame(table(cd$loci))
new%>%filter(Freq>1)
...
```{r}
#uploaded to slack random channel
```
