...
![(Image courtesy of RStudio27. CC-BY-4.0 license28.)](ggplot2-cheatsheetb.png)
...
```{r}
p_test<-p+geom_text(aes(populaiton/10^6,total,label=abb))
```
...
```{r}
p + geom_point(aes(col=region),size=3) + geom_abline(intercept = log10(r),slope=1)
```
