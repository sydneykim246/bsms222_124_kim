...
```{r}
# d246 is the data imported through rtracklayer

#calculate average
mean(d246%>%filter(type=="gene")%>%pull(width))
```
...
d246%>%filter(sequence!="chrM",type=="gene")%>%
  mutate(chrom_type=ifelse(sequence%in%c("chrx","chrY"),"sex_chr","autosomal_chr"))%>%
  group_by(chrom_type)%>%
  summarize(zero_to_hundred(width))
